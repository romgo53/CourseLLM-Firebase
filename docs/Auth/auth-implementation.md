...
- `FIREBASE_SERVICE_ACCOUNT_JSON` — the raw JSON string of a Firebase service account (or use `FIREBASE_SERVICE_ACCOUNT_PATH` to point at a file). Required for minting custom tokens with the Admin SDK (used for auth backend).
...
## User auth on micro-services

If you build micro-services that need to authenticate users, the recommended approach is(python example):

1. On the client, get the Firebase ID token:
   ...
   if (user) {
     ...
     // Call your micro-service with the ID token in the Authorization header
     const res = await fetch("/{path-to-your-micro-service}", {
       method: "POST",
       headers: {
...
2. On the micro-service, verify the ID token using the Firebase Admin SDK:
   show how to set the Admin SDK
...
```python
 ...
 def initialize_firebase_admin():
     ...
     cred = credentials.Certificate('path/to/serviceAccountKey.json') // Or the JSON object directly
 ...
 def verify_token(id_token):
  ...
  if __name__ == "__main__":
      ...
      // The id_token would come from the client request header "Authorization: Bearer <id_token>"
      verify_token("your_id_token_here")

```

3. Use `uid` to identify the user role (customUserClaims):
